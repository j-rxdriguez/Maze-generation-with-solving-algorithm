...
# Colour changes to greeen so the return path is identified.         
            retrn_cell(x, y) 
def return_path(x,y):
#Cell_solve contains the path to return to the starting point
    cell_solve(x, y)       
#Will loop until the cell position is euql to the start position
    while (x, y) != (20,20):    
    #new solution                                
        x, y = solution[x, y]  
        #Displays the way back                                  
        cell_solve(x, y)                                     
        time.sleep(.1)                              
...
maze_paths(x,y)  
return_path(400, 400)         # call the plot solution function             
